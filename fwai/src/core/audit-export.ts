...
/** Quote and escape a CSV field, guarding against delimiter breakage and formula injection. */
function csvField(value: string | number): string {
  let s = String(value);
  // Neutralize spreadsheet formula injection (=, +, -, @, tab, CR).
  if (/^[=+\-@\t\r]/.test(s)) s = `'${s}`;
  // Always quote and double internal quotes so commas/newlines can't shift columns.
  return `"${s.replace(/"/g, '""')}"`;
}
...
export function exportAsHtml(evidence: Evidence[], projectName?: string): string {
  ...
  // Evidence fields (run_id, skill, status) can contain attacker-influenced
  // text; escape everything interpolated into the report to prevent stored XSS.
  const rows = evidence
    .map((e) => {
      const status = escapeHtml(e.status);
      return `<tr><td>${escapeHtml(e.run_id)}</td><td>${escapeHtml(e.skill ?? "-")}</td><td class="${status}">${status}</td><td>${e.duration_ms}ms</td><td>${e.tools.length}</td></tr>`;
    })
    .join("\n");
...
/**
 * Compute a SHA-256 hash chain over ordered evidence for tamper detection.
 * Each record's hash folds in the previous record's hash, and fields are
 * length-delimited so distinct records can't be concatenated into a collision
 * (e.g. run_id "ab"+record "c" vs "a"+"bc").
 */
export function computeChainHash(evidence: Evidence[]): string {
  ...
  for (const e of evidence) {
    ...
    prev = crypto
      .createHash("sha256")
      .update(`${prev}\x1e${e.run_id}\x1e${serialized.length}\x1e${serialized}`)
      .digest("hex");
  }
  return prev || crypto.createHash("sha256").update("").digest("hex");
}
